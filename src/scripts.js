...
//EXPERIMENTO PARA FOTO DNI##################################

//ESPERA HASTA QUE OPENCV ESTÉ LISTO
...
function procesarDNI(imgElement, callback) {
    ...
    try {
        ...
        const finalBase64 = resultCanvas.toDataURL('image/jpeg', 0.7);
    ...
    // limpiar memoria (MUY IMPORTANTE)
    src.delete();
    gray.delete();
    blurred.delete();
    edged.delete();
    ...
    reader.onload = function(event) {
        ...
        img.onload = function() {
            ...
            waitForOpenCV(() => {
                ...
                procesarDNI(img, (resultado) => {

                    if (resultado) {
                        preview.src = resultado;
                        preview.classList.remove('hidden');
                        base64Input.value = resultado;
                    } else {
                        // fallback limpio
                        preview.src = img.src;
