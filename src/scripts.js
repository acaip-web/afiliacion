...
document.getElementById('dniFoto').addEventListener('change', function(e) {
    const file = e.target.files[0];
    if (!file) return;
    ...
    reader.onload = function(event) {
        const img = new Image();
        img.onload = function() {
            // Configuración del canvas para comprimir
            const canvas = document.createElement('canvas');
            const MAX_WIDTH = 1200; // Suficiente para leer datos pero ligero
            let width = img.width;
            let height = img.height;
            ...
            // Actualizar el formulario y la UI
            document.getElementById('dni-base64').value = dataUrl;
            document.getElementById('previewDni').src = dataUrl;
            document.getElementById('previewDni').classList.remove('hidden');
            document.getElementById('dni-placeholder').classList.add('opacity-0');
        };
        img.src = event.target.result;
    };
    reader.readAsDataURL(file);
